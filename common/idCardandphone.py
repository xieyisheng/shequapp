...
def getRandomPhone():
    phone= str(random.randint(15900000000, 15999999999))
    return phone

def getRandomName(typestr):
    name =typestr + str(random.randint(1, 10000))
    return name

def get_user_info(typestr):
    x=getRandomIdCard(sex=1)
    y=getRandomPhone()
    z=getRandomName(typestr)
    return dict(idcode=x,phone=y,name=z,type=typestr)

if __name__ == "__main__":
    #print(getRandomIdCard())
    #print(getRandomPhone())
    #print(getRandomName('业主转家属'))
    print(get_user_info('业主转家属'))
